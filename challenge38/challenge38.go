...
func Challenge38() {
	var solution int
	for n := 9000; n < 10_000; n++ {
		dbl := n * 2
		nSlice := shared.IntToIntSlice(n)
		dblSlice := shared.IntToIntSlice(dbl)
		nSlice = append(nSlice, dblSlice...)
		if !shared.ContainsDuplicates(nSlice) {
			candidateSolution := shared.IntSliceToInt(nSlice)
			if !shared.IntContainsDigit0(candidateSolution) {
				fmt.Printf("%v\n", nSlice)
				if candidateSolution > solution {
					solution = candidateSolution
				}
			}
		}
	}
	fmt.Printf("Challenge 38 solution is: %d\n", solution)
}
